...
if((length(args)==0) || args[1] == "--help" || args[1] == "-h" || !(isRStudio || args[1] == "--interactive" || (4 <= length(args)) && (length(args) <= 7))) {
  cat(paste("Usage:", 
            ...
            "  in case of 4 arguments: /path/to/copyNumbers-xxkb-bins.rds /path/to/OutputDir projectname dewavingKey (or n for no dewaving)",
...
# ... from CRAN
list.of.packages <- c("devtools", "R.cache", "MASS")
...
if(isRStudio || args[1]=="--interactive") {
  ...
  dewaveKeyNo <- ginput("'Dewaving' is currently an experimental and unpublished method in QDNAseq, based on Mark van de Wiel's NoWaves algorithm. If this would like this method to be applied to your data, provide the passcode to unlock this feature. Otherwise enter 'no'.", title="Dewaving", icon ="question")
  ...
  if (inclusionFileQuestion == "i") {
    ...
    args=c(copynumberfile, pathToOutputDir, projectname, dewaveKeyNo, inclusionFile)
    } else {args=c(copynumberfile, pathToOutputDir, projectname, dewaveKeyNo)}
}
...
if(dewaveKeyNo != 'no') {
  ...
  currentwd = getwd()
  setwd(tempdir())
  destfile = "dewaveCodeFragment.R"
  downloadurl = paste0("https://drive.google.com/uc?export=download&id=",dewaveKeyNo)
  x = getBinaryURL(downloadurl, followlocation = TRUE, ssl.verifypeer = FALSE)
